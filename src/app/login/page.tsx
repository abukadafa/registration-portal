...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      console.error(err);
      setError(
        err instanceof Error ? err.message : "Invalid credentials. Please double check and try again."
      );
    } finally {
      setSubmitting(false);
    }
            ...
            {isMock && process.env.NODE_ENV === "production" ? (
              ...
              <div className="rounded-xl bg-amber-50 p-4 ring-1 ring-amber-500/10 text-xs text-amber-800">
                ...
                <ul className="list-disc pl-4 mt-1 space-y-0.5 font-medium">
                  ...
                  <li>
                    Use <code className="font-mono bg-amber-100 px-1 rounded">finance@9aacaa.org</code> for Finance Officer access.
                  </li>
                  <li>
                    Use <code className="font-mono bg-amber-100 px-1 rounded">scan@9aacaa.org</code> for Attendance Officer access.
                  </li>
                </ul>
              </div>
            ) : (
