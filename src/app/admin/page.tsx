...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    if (!loading && !user) {
      ...
      // Standard "load data once auth resolves" effect; fetchData
      // internally toggles a loading flag, which this rule flags as
      // an indirect setState-in-effect even though it's the standard
      // async-fetch-on-mount pattern.
      // eslint-disable-next-line react-hooks/set-state-in-effect
      fetchData();
      if (!permissions.canViewRegistrations(user.role)) {
  ...
  // Authenticated with Firebase Auth, but no role document (or an
  // unrecognized role) in Firestore. Never grant implicit access here —
  // show a clear "contact an admin" screen instead.
  if (!user.role) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-[var(--color-cream)] px-6 text-center">
        <p className="font-[family-name:var(--font-display)] text-xl font-semibold text-[var(--color-ink)]">
          No portal access assigned
        </p>
        <p className="max-w-sm text-sm text-[var(--color-ink-soft)]">
          Your account ({user.email}) is signed in but doesn&rsquo;t have a role assigned yet.
          Ask an administrator to add you to the <code className="font-mono">users</code> collection
          with a role (admin, registration-officer, attendance-officer, or finance-officer).
        </p>
        <button
          onClick={() => signOut()}
  ...
  // Handle payment status update (Finance Officer + Admin)
  async function handlePaymentUpdate(id: string, newStatus: PaymentStatus) {
    try {
      await updateParticipant(id, { paymentStatus: newStatus });
      setParticipants((prev) =>
        prev.map((p) => (p.id === id ? { ...p, paymentStatus: newStatus } : p))
      );
      if (selectedParticipant && selectedParticipant.id === id) {
        setSelectedParticipant((prev) => (prev ? { ...prev, paymentStatus: newStatus } : null));
      }
    } catch (err) {
      alert("Failed to update payment status. Please check your credentials.");
      console.error(err);
    }
  }
  ...
  async function handleRestore(id: string) {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "Failed to restore participant.");
      console.error(err);
    }
  }
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "Failed to update participant details.");
      console.error(err);
    } finally {
  ...
  async function handleAddBulk() {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "Failed to bulk import participants.");
      console.error(err);
    } finally {
      ...
      <main className="flex-1 px-6 py-10 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
          ...
          <div className="flex gap-2">
            ...
            {activeTab === "registrations" && permissions.canRunReports(user.role) && (
              <button
        ...
        <div className="flex border-b border-black/10 mb-8 space-x-8">
          {permissions.canViewRegistrations(user.role) && (
            <button
          ...
          {permissions.canManageAttendance(user.role) && (
            <button
          ...
          {permissions.canAccessRecycleBin(user.role) && (
            <button
        ...
        {activeTab === "registrations" && permissions.canViewRegistrations(user.role) && (
          <>
            {/* Stats Grid */}
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs">
                      ...
                      <div>
                        ...
                        {permissions.canManagePayments(user.role) ? (
                          <select
                            className="mt-0.5 rounded-md border border-black/10 text-xs font-medium py-1 px-1.5 cursor-pointer"
                            value={selectedParticipant.paymentStatus}
                            onChange={(e) =>
                              handlePaymentUpdate(selectedParticipant.id, e.target.value as PaymentStatus)
                            }
                          >
                            <option value="Pending">Pending</option>
                            <option value="Paid">Paid</option>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        className="input"
                        value={newGender}
                        onChange={(e) => setNewGender(e.target.value as "Male" | "Female" | "Prefer not to say")}
                      >
                    ...
                    <div>
                      ...
                      <select
                        className="input"
                        value={newCategory}
                        onChange={(e) => setNewCategory(e.target.value as ParticipantCategory)}
                      >
                        <option value="Researcher">Researcher</option>
                        <option value="Policy Maker">Policy Maker</option>
                        <option value="Industry / Private Sector">Industry / Private Sector</option>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      className="input"
                      value={editGender}
                      onChange={(e) => setEditGender(e.target.value as "Male" | "Female" | "Prefer not to say")}
                    >
                      <option value="Male">Male</option>
                      <option value="Female">Female</option>
                      <option value="Prefer not to say">Prefer not to say</option>
                  ...
                  <div>
                    ...
                    <select
                      className="input"
                      value={editCategory}
                      onChange={(e) => setEditCategory(e.target.value as ParticipantCategory)}
                    >
                      <option value="Researcher">Researcher</option>
                      <option value="Policy Maker">Policy Maker</option>
                      <option value="Industry / Private Sector">Industry / Private Sector</option>
                ...
                <div className="grid grid-cols-3 gap-3 border-t border-black/5 pt-4">
                  <div>
                    ...
                    <select
                      ...
                      value={editPayment}
                      disabled={!permissions.canManagePayments(user.role)}
                      title={
                        permissions.canManagePayments(user.role)
                          ? undefined
                          : "Only Finance Officers and Admins can change payment status"
                      }
